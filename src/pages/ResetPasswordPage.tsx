import { ArrowLeft, Eye, EyeOff } from 'lucide-react'; // Đảm bảo bạn đã cài lucide-react
...
const ResetPasswordPage: React.FC = () => {
  ...
  // username can be passed via location.state or query params
  const passedUsername = (location.state as any)?.username || new URLSearchParams(location.search).get('username') || '';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!passedUsername) {
        alert('Không có thông tin tài khoản để khôi phục. Vui lòng bắt đầu từ trang đăng nhập.');
        return;
      }
      // gọi API công khai reset password theo username
      await api.authResetPassword(passedUsername, password);
      alert('Cập nhật mật khẩu thành công! Vui lòng đăng nhập bằng mật khẩu mới.');
      navigate('/login');
    } catch (error) {
      ...
      <div className="max-w-[480px] w-full bg-white rounded-2xl shadow-2xl p-6 md:p-8 animate-fade-in-up relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input
              type="text"
              value={passedUsername}
              readOnly
              className="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-100 text-slate-700"
            />
          </div>
          {/* Mật khẩu mới */}
          <div>
