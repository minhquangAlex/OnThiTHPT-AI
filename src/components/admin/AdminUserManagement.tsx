...
const AdminUserManagement: React.FC = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Backend now returns { users (with attemptCount), total, page, limit }
      const list = resp?.users || resp?.data || [];
      setUsers(list);
      setTotal(resp?.total ?? null);
      console.log('✅ [AdminUserManagement] Users loaded:', list);
    } catch (err: any) {
      console.error('❌ [AdminUserManagement] Không thể tải users', err);
      setUsers([]);
    } finally {
      setLoading(false);
    }
          ...
          <tbody>
            {filtered.map(u => (
              <tr key={u._id} className="border-b border-slate-700">
                ...
                <td className="p-2">{u.attemptCount ?? 0}</td>
