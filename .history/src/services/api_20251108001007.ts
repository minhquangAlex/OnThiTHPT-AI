...
const api = {
    ...
    getQuestions: async (idOrSlug: string): Promise<Question[]> => {  // Param generic
    console.log(`\x1b[32m[DEBUG Frontend] Fetching questions for param: '${idOrSlug}' (likely ${idOrSlug.length === 24 ? 'ID' : 'slug'})\x1b[0m`);
    const response = await fetch(`${API_URL}/questions/${idOrSlug}`, {
        // headers...
    });
    if (!response.ok) {
    ...
    console.log(`\x1b[32m[DEBUG Frontend] API Response: ${result.data?.length || 0} questions\x1b[0m`);
    return result.data || [];
},
