...
const api = {
    ...
    lookupUser: async (username: string) => {
        const response = await fetch(`${API_URL}/auth/lookup?username=${encodeURIComponent(username)}`, {
            method: 'GET',
    ...
    publicResetPassword: async (data: { userId?: string; username?: string; password: string }) => {
    ...
    updateUser: async (userId: string, data: { name?: string; role?: string; email?: string; className?: string; school?: string }) => {
