...
const api = {
  ...
  if (!response.ok) throw new Error('Không thể tải danh sách lượt làm bài');
  const result = await response.json();
  return result.data || [];  // Handle wrap
},
  ...
  if (!response.ok) throw new Error('Không thể tải chi tiết lượt làm bài');
  const result = await response.json();
  return result.data || result;  // Handle wrap
},
    ...
    // Thêm postAttempt
postAttempt: async (subjectId: string, answers: { questionId: string; selectedAnswer: string }[]) => {
  const token = getAuthToken();
  if (!token) throw new Error('Cần đăng nhập để nộp bài');
  console.log(`\x1b[32m[DEBUG Frontend] postAttempt: subjectId '${subjectId}', answers ${answers.length}\x1b[0m`);
  const response = await fetch(`${API_URL}/attempts`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`,
  ...
  if (!response.ok) {
    const err = await response.json();
    throw new Error(err.message || 'Lỗi nộp bài');
  }
  const result = await response.json();
  const attempt = result.data || result;  // Handle wrap
  console.log(`\x1b[32m[DEBUG Frontend] postAttempt success: ID ${attempt._id}, score ${attempt.score}\x1b[0m`);
  return attempt._id;  // Return ID cho navigate
},
