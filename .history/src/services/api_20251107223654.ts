...
const api = {
    ...
    getQuestions: async (subjectSlug: string): Promise<Question[]> => {
        const token = getAuthToken();
        const response = await fetch(`${API_URL}/questions/${subjectSlug}`, {
            headers: {
                // In a real app, you might protect this route
                // 'Authorization': `Bearer ${token}`,
            },
        });
        if (!response.ok) {
    ...
    getQuestionStats: async (subjectSlug?: string) => {
      const token = getAuthToken();
      const response = await fetch(`${API_URL}/stats/questions/${subjectSlug || ''}`, {
        headers: { 
          Authorization: `Bearer ${token}`,
        },
      });
      if (!response.ok) {
        ...
        try {
          const parsed = JSON.parse(bodyText || '{}');
          throw new Error(parsed.message || bodyText || 'Không thể tải thống kê câu hỏi');
        } catch (e) {
          throw new Error(bodyText || 'Không thể tải thống kê câu hỏi');
        }
      }
      return response.json();
    },
