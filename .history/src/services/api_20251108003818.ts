...
const api = {
    ...
    getQuestions: async (subjectId: string): Promise<Question[]> => {  // Chỉ dùng cho ID
    ...
    if (!response.ok) {
        ...
        console.error(`\x1b[32m[DEBUG Frontend] API Error (${response.status}): ${errorText}\x1b[0m`);
        const errorData = JSON.parse(errorText || '{}');
        throw new Error(errorData.message || 'Không thể tải câu hỏi');
    }
    const result = await response.json();
    const questions = result.data || result || [];  // Fix: Handle wrap hoặc direct array, tránh undefined
    console.log(`\x1b[32m[DEBUG Frontend] API Response: ${questions.length} questions\x1b[0m`, questions.slice(0, 1));  // Log preview 1 question
    return questions;
},
