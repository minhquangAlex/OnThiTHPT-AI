...
const AccountSettings: React.FC<AccountSettingsProps> = ({ user }) => {
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      // Gọi API reset password (hoặc change password tùy backend)
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.warn('⚠️ Lỗi cập nhật store:', setErr);
        }
        ...
        <form onSubmit={handlePasswordChange} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <input
                type={showCurrentPass ? 'text' : 'password'} // Đổi type
                value={passwordData.currentPassword}
                onChange={(e) => setPasswordData({ ...passwordData, currentPassword: e.target.value })}
                className="w-full pl-4 pr-10 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                required
              />
              <button
                type="button"
