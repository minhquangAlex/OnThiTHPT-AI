...
const AccountSettings: React.FC<AccountSettingsProps> = ({ user }) => {
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      try {
        const res = await api.updateUser(user.id || user._id, { name: profileData.name });
        // Update local store user so ProfileInfo reflects changes immediately
        try {
          const updatedUser = res.user || res;
          setUser({ ...(user as any), ...(updatedUser as any), name: profileData.name, className: profileData.className, school: profileData.school });
        } catch (setErr) {
          // ignore
        }
