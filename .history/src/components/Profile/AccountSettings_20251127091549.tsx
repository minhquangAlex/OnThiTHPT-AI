...
const AccountSettings: React.FC<AccountSettingsProps> = ({ user }) => {
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      console.log('🔵 [handleProfileUpdate] Bắt đầu cập nhật profile');
      console.log('📨 [handleProfileUpdate] Dữ liệu gửi đi:', profileData);
      
        try {
        console.log('📤 [handleProfileUpdate] Gọi api.updateUser với userId:', user.id || user._id);
        const res = await api.updateUser(user.id || user._id, { 
        ...
        console.log('✅ [handleProfileUpdate] Response từ server:', res);
        
        // Update local store user so ProfileInfo reflects changes immediately
        try {
          const updatedUser = res.user || res;
          console.log('💾 [handleProfileUpdate] Cập nhật local store, user object:', updatedUser);
          setUser({ ...(user as any), ...(updatedUser as any), name: profileData.name, email: profileData.email, className: profileData.className, school: profileData.school });
          console.log('✅ [handleProfileUpdate] Đã cập nhật local store xong');
        } catch (setErr) {
          console.warn('⚠️ [handleProfileUpdate] Lỗi cập nhật store:', setErr);
          // ignore
        }
        setMessage({ type: 'success', text: 'Thông tin cá nhân đã được cập nhật' });
      } catch (updateError: any) {
        console.error('❌ [handleProfileUpdate] Lỗi API:', updateError);
        // Nếu API cập nhật không khả dụng, hiển thị thông báo thân thiện
        if (updateError.message?.includes('403') || updateError.message?.includes('Forbidden')) {
          setMessage({ 
            type: 'error', 
            text: 'Chức năng cập nhật thông tin hiện không khả dụng. Hãy liên hệ admin để cập nhật.' 
          });
        } else {
          throw updateError;
        }
      ...
      console.error('❌ [handleProfileUpdate] Lỗi chung:', error);
