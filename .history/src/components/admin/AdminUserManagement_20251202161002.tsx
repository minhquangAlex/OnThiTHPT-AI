...
const AdminUserManagement: React.FC = () => {
  ...
  const [usersWithAttempts, setUsersWithAttempts] = useState<Map<string, number>>(new Map());
  ...
  const handleToggleBan = async (userId: string, ban: boolean) => {
    if (!window.confirm(ban ? 'Khóa tài khoản này?' : 'Mở khóa tài khoản này?')) return;
    try {
      await api.updateUser(userId, { role: undefined });
      // For demo, we store banned state client-side: in real app have status field
  ...
  const filtered = users.filter(u => {
    ...
    // statusFilter is a placeholder; assume active if !banned
    return true;
  });
          ...
          <tbody>
            {filtered.map(u => (
              <tr key={u._id} className="border-b border-slate-700">
                ...
                <td className="p-2">{u.attemptCount ?? '-'}</td>
                <td className="p-2 space-x-2">
                  ...
                  <Button size="sm" variant="secondary" onClick={() => handleToggleBan(u._id, true)}>Khóa</Button>
