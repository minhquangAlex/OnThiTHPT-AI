  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Tính toán thống kê tổng quan (chuẩn hoá trên thang 0..10)
        const totalAttempts = userAttempts.length;
        let totalCorrect = 0;
        let totalQuestions = 0;

        // build per-subject performance map to compute subject averages
        ...
        userAttempts.forEach((attempt: any) => {
          ...
          const subjectId = String(attempt.subject || attempt.subjectId || 'unknown');
          if (!performanceMap.has(subjectId)) performanceMap.set(subjectId, { correct: 0, total: 0 });
          const entry = performanceMap.get(subjectId)!;
          entry.correct += s.correct;
          entry.total += s.total;
        });

        // average across subjects (per-subject averages), ignore subjects with total=0
        const subjectAverages: number[] = [];
        performanceMap.forEach((val, key) => {
          if (val.total > 0) {
            const avg10 = (val.correct / val.total) * 10;
            subjectAverages.push(Math.round(avg10 * 10) / 10);
          }
        });

        const averageScore = subjectAverages.length > 0 ? Math.round((subjectAverages.reduce((a, b) => a + b, 0) / subjectAverages.length) * 10) / 10 : 0;
