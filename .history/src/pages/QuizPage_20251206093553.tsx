...
const getFullImageUrl = (imagePath?: string) => {
  ...
  if (imagePath.startsWith('http')) return imagePath; // Nếu đã là link online thì giữ nguyên

  // Lấy link backend gốc từ biến môi trường hoặc hardcode link Ngrok của bạn
  // Lưu ý: API_URL thường có đuôi /api, ta cần cắt bỏ nó để lấy root domain
  // Ví dụ: ...ngrok.dev/api -> ...ngrok.dev
  const API_URL = (import.meta as any).env.VITE_API_URL || 'https://undisputedly-nonsocialistic-sheba.ngrok-free.dev/api';
  const rootUrl = API_URL.replace(/\/api$/, ''); // Xóa chữ /api ở cuối

  // Kết quả: https://...ngrok.dev/uploads/filename.jpg
