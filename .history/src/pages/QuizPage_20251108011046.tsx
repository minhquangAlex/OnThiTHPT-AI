...
const QuizPage: React.FC = () => {
  ...
  // Thêm hàm submitQuiz local (nếu store chưa có, hoặc call store nếu có)
  const submitQuizLocal = async (subjectId: string) => {
    if (!answers.length) throw new Error('Chưa trả lời câu nào');
    try {
      const attemptId = await api.postAttempt(subjectId, answers);  // Gọi api mới
      return attemptId;
    } catch (error) {
      console.error('\x1b[32m[DEBUG Frontend] submitQuiz error:\x1b[0m', error);
      throw error;  // Re-throw để handle
    }
  };
