...
import { Image as ImageIcon, CheckSquare, Square, Trash2, Unlink } from 'lucide-react'; // Thêm Unlink
...
const ExamQuestionsPage: React.FC = () => {
  ...
  const handleBulkRemove = async () => {
    ...
    try {
        // Dùng Promise.all để gọi API gỡ từng câu (Frontend xử lý vòng lặp)
        await Promise.all(Array.from(selectedIds).map(qId => api.removeQuestionFromExam(examId!, qId)));
        
        // Cập nhật giao diện: Loại bỏ các câu đã gỡ
        ...
        alert(err.message || 'Lỗi khi gỡ câu hỏi. Vui lòng thử lại.');
        // Load lại trang để đồng bộ dữ liệu nếu có lỗi mạng
            ...
            {!isSelectionMode ? (
                ...
                <>
                    ...
                    <Button variant="secondary" onClick={toggleSelectAll}>
                        {selectedIds.size === questions.length ? 'Bỏ chọn' : 'Tất cả'}
                    </Button>
